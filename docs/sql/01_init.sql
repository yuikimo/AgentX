...
-- 关键词检索配置（混合检索）
-- 强制要求 jieba parser：若 pg_jieba 不可用，初始化直接失败
DO $$
DECLARE
    token_aliases text;
BEGIN
    ...
    EXECUTE 'DROP TEXT SEARCH CONFIGURATION IF EXISTS public.chinese_cfg';
    EXECUTE 'CREATE TEXT SEARCH CONFIGURATION public.chinese_cfg (PARSER = jieba)';
    SELECT string_agg(alias, ',')
    INTO token_aliases
    FROM ts_token_type('jieba');

    IF token_aliases IS NULL OR token_aliases = '' THEN
        RAISE EXCEPTION 'jieba parser has no token aliases';
    END IF;

    EXECUTE format(
            'ALTER TEXT SEARCH CONFIGURATION public.chinese_cfg ADD MAPPING FOR %s WITH simple',
            token_aliases
    );
END
$$;
...
insert into public.container_templates (
    ...
    'default-user-mcp-gateway-template',
    '用户MCP网关默认模板',
    '用于创建用户MCP网关容器的默认模板',
    'USER',
    'my-mcp-gateway',
...
insert into public.container_templates (
    ...
    'default-review-gateway-template',
    '审核容器默认模板',
    '用于工具审核环境的默认容器模板',
    'REVIEW',
    'my-mcp-gateway',
...
comment on column public.messages.attachments is '消息附件详情，包含url/name/contentType/kind/summary';
