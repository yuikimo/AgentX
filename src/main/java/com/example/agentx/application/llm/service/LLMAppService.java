...
public class LLMAppService {
    ...
    public ProviderDTO getProviderDetail(String providerId, String userId) {
        return getProviderDetail(providerId, userId, false);
    }

    /** 获取服务商聚合根，可选择是否返回真实密钥。仅用户自定义服务商允许返回真实密钥。 */
    public ProviderDTO getProviderDetail(String providerId, String userId, boolean includeSecret) {
        ProviderAggregate providerAggregate = llmDomainService.getProviderAggregate(providerId, userId);
        boolean maskSensitive = !includeSecret || Boolean.TRUE.equals(providerAggregate.getIsOfficial());
        return ProviderAssembler.toDTO(providerAggregate, maskSensitive);
    }
    ...
    public ModelDTO createModel(ModelCreateRequest modelCreateRequest, String userId) {
        ...
        // 如果用户没有默认模型则设置当前模型
        if (userDefaultModelId == null && model.isChatType()) {
    ...
    public boolean canUserUseModel(String modelId, String userId) {
        try {
            ...
            // 官方模型 + 激活状态 = 可用
            if (Boolean.TRUE.equals(model.getOfficial()) && Boolean.TRUE.equals(model.getStatus())) {
                return true;
            }
            // 用户自己的模型 + 激活状态 = 可用
            if (userId.equals(model.getUserId()) && Boolean.TRUE.equals(model.getStatus())) {
                return true;
            }
            return false;
        } catch (Exception e) {
