...
public class PreviewMessageHandler extends AbstractMessageHandler {
    ...
    public PreviewMessageHandler(LLMServiceFactory llmServiceFactory, MessageDomainService messageDomainService,
            ...
            AgentToolManager agentToolManager, ProviderConfigFactory providerConfigFactory,
    ...
    protected <T> void processChat(Agent agent, T connection, MessageTransport<T> transport, ChatContext chatContext,
            ...
            com.example.agentx.application.conversation.service.message.ToolProgressStreamState toolProgressStreamState) {

        AtomicReference<StringBuilder> messageBuilder = new AtomicReference<>(new StringBuilder());

        TokenStream tokenStream = agent.chat(buildCurrentUserMessage(chatContext));

        tokenStream.onError(throwable -> {
            transport.sendMessage(connection, ChatErrorResponseFactory.fromThrowable(throwable));
        ...
        tokenStream.onCompleteResponse(chatResponse -> {
            ...
            // 执行模型调用计费
            performBillingWithErrorHandling(chatContext, ChatResponseTokenUsageUtils.inputTokenCount(chatResponse),
        ...
        tokenStream.onToolExecuted(toolExecution -> {
            if (messageBuilder.get().length() > 0) {
                boolean alreadyFlushed = toolProgressStreamState != null
            ...
            } else if (toolProgressStreamState != null) {
                toolProgressStreamState.consumeTextSegmentFlushed();
            }
            ...
            toolMessage.setMetadata(buildToolExecutionPayload(toolExecution.request().arguments(), toolExecution.result(),
                    null));
            transport.sendMessage(connection,
                    buildToolExecutionResponse(message, toolExecution.request().arguments(), toolExecution.result()));
        });
