...
import com.example.agentx.application.conversation.service.message.agent.analysis.dto.AnalyzerMessageDTO;
...
public class AnalyserMessageHandler extends AbstractAgentHandler {
    ...
    private <T> ChatRequest buildRequest(AgentWorkflowContext<T> context) {
        ...
        PromptSpec promptSpec = AgentWorkflowPromptTemplates
                .buildAnalyserMessagePromptSpec(chatContext.getUserMessage());
        List<ChatMessage> messages = new ArrayList<>();

        String summary = ConversationPromptContextUtils.resolveNormalizedSummary(chatContext.getMessageHistory(),
                chatContext.getContextEntity());
        String baseSystemPrompt = new PromptContextBuilder()
                .persona(chatContext.getAgent() != null ? chatContext.getAgent().getSystemPrompt() : null)
                .memory(StringUtils.isNotBlank(summary) ? ConversationPromptTemplates.wrapConversationSummary(summary) : null)
                .build();
        if (StringUtils.isNotBlank(baseSystemPrompt)) {
            messages.add(new SystemMessage(baseSystemPrompt));
        }

        for (MessageEntity messageEntity : ConversationHistoryUtils.stripSummaryMessages(chatContext.getMessageHistory())) {
            String content = messageEntity.getContent();
            if (messageEntity.getRole() == Role.USER) {
                messages.add(new UserMessage(content));
            } else if (messageEntity.getRole() == Role.ASSISTANT || messageEntity.getRole() == Role.SYSTEM) {
                messages.add(new AiMessage(content));
            }
        }

        if (!promptSpec.getSystemPrompt().isBlank()) {
