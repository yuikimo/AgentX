...
public class UserRagDomainService {
    ...
    public UserRagDomainService(UserRagRepository userRagRepository, RagVersionDomainService ragVersionDomainService,
                                ...
                                UserRagSnapshotDomainService userRagSnapshotService) {
    ...
    public UserRagEntity installRag(String userId, String ragVersionId) {
        ...
        if ("0.0.1".equals(ragVersion.getVersion()) && !ragVersion.getUserId().equals(userId)) {
            throw new BusinessException("0.0.1版本未作者私有版本，不支持安装");
        }

        // 如果是自己的RAG，允许安装任何版本（包括私有版本）
    ...
    public IPage<UserRagEntity> listInstalledRags(String userId, Integer page, Integer pageSize, String keyword) {
        ...
        if (StringUtils.isNotBlank(keyword)) {
            wrapper.and(w -> w.like(UserRagEntity::getName, keyword)
                    .or()
                    .like(UserRagEntity::getDescription, keyword)
            );
        }
     ...
     * @param icon          新图标
     */
    public void updateUserRagBasicInfo(String userId, String originalRagId, String name,
                                       String description, String icon) {
        LambdaUpdateWrapper<UserRagEntity> wrapper = Wrappers.<UserRagEntity>lambdaUpdate()
                .eq(UserRagEntity::getUserId, userId)
                .eq(UserRagEntity::getOriginalRagId, originalRagId)
                // 只更新REFERENCE类型的安装
                .eq(UserRagEntity::getInstallType, InstallType.REFERENCE)
    ...
    public List<UserRagEntity> getAvailableVersionsByUserRagId(String userId, String userRagId) {
        ...
        // 获取该用户已安装的版本
        List<UserRagEntity> installVersions = getInstalledVersionsByUserRagId(userId, userRagId);
        Map<String, UserRagEntity> installedVersionMap =
                installVersions.stream().collect(Collectors.toMap(UserRagEntity::getRagVersionId, entity -> entity));

        // 获取该RAG的版本列表（根据用户权限决定范围）
        List<RagVersionEntity> availableVersions =
                ragVersionDomainService.getVersionsByOriginalRagId(currentUserRag.getOriginalRagId(), userId);
        ...
        for (RagVersionEntity ragVersion : availableVersions) {
            UserRagEntity currentUserRagEntity;
            ...
            currentUserRagEntity.setInstallType(!Objects.equals(ragVersion.getVersion(), "0.0.1") ?
                    InstallType.SNAPSHOT : InstallType.REFERENCE);
