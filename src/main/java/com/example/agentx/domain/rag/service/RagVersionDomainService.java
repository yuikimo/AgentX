...
import com.example.agentx.application.rag.dto.RagVersionDocumentDTO;
import com.example.agentx.application.rag.dto.RagVersionFileDTO;
...
import com.example.agentx.domain.rag.repository.RagVersionDocumentRepository;
import com.example.agentx.domain.rag.repository.RagVersionFileRepository;
import com.example.agentx.domain.rag.repository.RagVersionRepository;
...
public class RagVersionDomainService {
    ...
    public IPage<RagVersionEntity> listUserVersions(String userId, Integer page, Integer pageSize, String keyword) {
        ...
        if (StringUtils.isNotBlank(keyword)) {
            wrapper.and(w -> w.like(RagVersionEntity::getName, keyword)
                    .or()
                    .like(RagVersionEntity::getDescription, keyword));
        }
    ...
    public IPage<RagVersionEntity> listPublishedVersions(Integer page, Integer pageSize, String keyword) {
        ...
        Map<String, RagVersionEntity> latestMap = allPublishedList.stream()
    ...
    public IPage<RagVersionEntity> listAllVersions(Integer page, Integer pageSize, String keyword, Integer status) {
        LambdaQueryWrapper<RagVersionEntity> wrapper = Wrappers.lambdaQuery();
        if (StringUtils.isNotBlank(keyword)) {
            wrapper.and(w -> w.like(RagVersionEntity::getName, keyword)
                    .or()
                    .like(RagVersionEntity::getDescription, keyword)
    ...
    public RagContentPreviewDTO getRagContentPreview(String versionId) {
        ...
        Map<String, String> fileNameMap = files.stream()
                .collect(Collectors.toMap(RagVersionFileEntity::getId, RagVersionFileEntity::getFileName,
                        (k1, k2) -> k1)
                );
        ...
        List<RagVersionDocumentDTO> docDTOs = allDocs.stream()
                .map(doc -> {
                    RagVersionDocumentDTO dto = new RagVersionDocumentDTO();
                    BeanUtils.copyProperties(doc, dto);
                    // 获取文件名
                    dto.setFileName(fileNameMap.getOrDefault(doc.getRagVersionFileId(), null));
                    return dto;
                })
                .collect(Collectors.toList());
