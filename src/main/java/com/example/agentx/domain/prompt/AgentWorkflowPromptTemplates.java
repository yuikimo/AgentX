...
/** Agent 工作流提示词模板 */
public final class AgentWorkflowPromptTemplates {

    private static final String INFO_ANALYSIS_PROMPT = "你是一个专业的任务规划助手，你的目标是提供友好且全面的分析服务。请先仔细分析用户提出的目标或问题，并判断是否具备实现目标所必需的关键信息。\n\n"
    ...
    private static final String DECOMPOSITION_PROMPT = "你是一个任务规划专家，负责将用户的目标或问题拆解成一系列清晰、独立且具备执行框架的子任务。\n\n"
            + "任务拆分指南：\n"
    ...
    private static final String TASK_EXECUTION_PROMPT = "你是一个专注、高效且富有洞察力的任务执行专家。现在你的任务是根据给定的上下文完成一个特定的子任务。\n\n"
            + "执行指南：\n"
    ...
    private static final String SUMMARY_PROMPT = "你是一个擅长整合信息并提供个性化回应的总结专家。现在，你需要将所有子任务的成果融合成一个连贯、全面且引人入胜的最终回答。\n\n"
    ...
    private static final String ANALYSER_MESSAGE_PROMPT = "你是一个敏锐且善于理解用户意图的分析助手。你需要准确判断用户的消息是简单的问答交流还是需要规划执行的复杂任务。\n\n"
    ...
    private AgentWorkflowPromptTemplates() {
    }

    public static PromptSpec buildInfoAnalysisPromptSpec() {
        return PromptSpec.of(INFO_ANALYSIS_PROMPT, "");
    }

    public static PromptSpec buildAnalyserMessagePromptSpec(String userMessage) {
        return PromptSpec.of(ANALYSER_MESSAGE_PROMPT, "用户消息是：\n" + userMessage);
    }

    public static PromptSpec buildDecompositionPromptSpec(String userMessage) {
        return PromptSpec.of(DECOMPOSITION_PROMPT, userMessage);
    }

    public static PromptSpec buildTaskExecutionPromptSpec(String userRequest, String currentTask,
            Map<String, String> previousTaskResults) {
        return buildTaskExecutionPromptSpec(userRequest, currentTask, previousTaskResults, 0, "", "", "");
    }

    public static PromptSpec buildTaskExecutionPromptSpec(String userRequest, String currentTask,
            Map<String, String> previousTaskResults, int maxToolCallsPerTurn, String toolCatalogPrompt,
            String presetToolPrompt, String toolAvailabilityNotice) {
        ...
        if (previousTaskResults != null && !previousTaskResults.isEmpty()) {
            ...
            previousTaskResults.forEach((task, result) -> previousTasksBuilder.append("- 任务: ").append(task)
                    .append("\n  结果: ").append(result).append("\n"));
            previousTasksBuilder.append("\n");
        }
        String systemPrompt = new PromptContextBuilder()
                .persona(TASK_EXECUTION_PROMPT)
                .capabilities(ConversationPromptTemplates.generateToolPolicyPrompt(maxToolCallsPerTurn))
                .capabilities(toolCatalogPrompt)
                .capabilities(presetToolPrompt)
                .constraints(ConversationPromptTemplates.wrapToolAvailabilityNotice(toolAvailabilityNotice))
                .build();
        return PromptSpec.of(systemPrompt,
                String.format("请仔细阅读以下信息：\n原始用户请求: %s\n\n当前需要完成的子任务: %s\n\n%s", userRequest,
                        currentTask, previousTasksBuilder));
    }

    public static PromptSpec buildSummaryPromptSpec(String taskResults) {
        return PromptSpec.of(String.format(SUMMARY_PROMPT, taskResults), "请基于上述子任务结果提供总结");
    }
}
