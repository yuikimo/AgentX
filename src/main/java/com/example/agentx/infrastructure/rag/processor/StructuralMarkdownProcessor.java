...
import com.example.agentx.domain.rag.model.ProcessedSegment;
import com.example.agentx.domain.rag.model.enums.SegmentType;
...
public class StructuralMarkdownProcessor implements MarkdownProcessor {
    ...
    public List<ProcessedSegment> processRawSegments(String markdown, ProcessingContext context) {
        if (markdown == null || markdown.trim().isEmpty()) {
            return new ArrayList<>();
        }
        ...
        try {
            ...
            log.error("结构化处理器原文处理Markdown失败", e);
            ProcessedSegment fallback = new ProcessedSegment(markdown, SegmentType.TEXT, null);
            fallback.setOrder(0);
            return List.of(fallback);
        }
