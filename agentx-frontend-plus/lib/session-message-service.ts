...
export interface QuerySessionMessagesRequest {
  page?: number
  pageSize?: number
}
...
// 分页获取会话消息，默认按最近消息优先返回
export async function getSessionMessagesPage(
  sessionId: string,
  params?: QuerySessionMessagesRequest
): Promise<ApiResponse<PageResponse<MessageDTO>>> {
  try {
    const data = await httpClient.get<ApiResponse<PageResponse<MessageDTO>>>(
      API_ENDPOINTS.SESSION_MESSAGES_PAGE(sessionId),
      { params }
    )
    return data
  } catch (error) {
