...
// 批量删除（归档）记忆
export async function batchDeleteMemory(request: DeleteMemoryBatchRequest): Promise<ApiResponse<number>> {
  try {
    const response = await httpClient.post<ApiResponse<number>>(
      API_ENDPOINTS.BATCH_DELETE_MEMORY,
      request
    );
    return response;
  } catch (error) {
...
export const batchDeleteMemoryWithToast = withToast(batchDeleteMemory, {
  showSuccessToast: true,
  showErrorToast: true,
  successTitle: "批量删除记忆成功",
  errorTitle: "批量删除记忆失败",
});
