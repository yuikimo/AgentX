...
import type {
  ...
  SwitchDatasetEmbeddingModelRequest,
  QueryDatasetRequest,
...
// 切换数据集嵌入模型（异步迁移）
export async function switchDatasetEmbeddingModel(
  datasetId: string,
  request: SwitchDatasetEmbeddingModelRequest
): Promise<ApiResponse<RagDataset>> {
  try {
    const response = await httpClient.put<ApiResponse<RagDataset>>(
      API_ENDPOINTS.RAG_SWITCH_DATASET_EMBEDDING_MODEL(datasetId),
      request
    )
    return response
  } catch (error) {
...
// 重试数据集嵌入迁移
export async function retryDatasetEmbeddingMigration(datasetId: string): Promise<ApiResponse<RagDataset>> {
  try {
    const response = await httpClient.post<ApiResponse<RagDataset>>(
      API_ENDPOINTS.RAG_RETRY_DATASET_EMBEDDING_MIGRATION(datasetId),
      {}
    )
    return response
  } catch (error) {
...
export async function reprocessFile(request: ProcessFileRequest): Promise<ApiResponse<void>> {
  try {
    ...
    const response = await httpClient.post<ApiResponse<void>>(
      API_ENDPOINTS.RAG_REPROCESS_FILE,
      request
    )
...
export const switchDatasetEmbeddingModelWithToast = withToast(switchDatasetEmbeddingModel, {
  successTitle: "已开始切换嵌入模型",
  errorTitle: "切换嵌入模型失败"
})

export const retryDatasetEmbeddingMigrationWithToast = withToast(retryDatasetEmbeddingMigration, {
  successTitle: "已重新触发嵌入迁移",
  errorTitle: "重试嵌入迁移失败"
})
...
export const reprocessFileWithToast = withToast(reprocessFile, {
  successTitle: "重启文件预处理成功",
  errorTitle: "重启文件预处理失败"
})
