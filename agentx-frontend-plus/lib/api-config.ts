...
function getDefaultApiUrl(): string {
  ...
  if (configuredApiBaseUrl && configuredApiBaseUrl !== 'auto') {
...
export const API_ENDPOINTS = {
  ...
  SESSION_MESSAGES_PAGE: (id: string) => `/agents/sessions/${id}/messages/page`,
  ...
  CONFIGURE_INSTALLED_TOOL: (toolId: string) => `/tools/installed/${toolId}/config`,
  ...
  RAG_SWITCH_DATASET_EMBEDDING_MODEL: (id: string) => `/rag/datasets/${id}/embedding-model`, // 切换数据集嵌入模型
  RAG_RETRY_DATASET_EMBEDDING_MIGRATION: (id: string) => `/rag/datasets/${id}/embedding-model/retry`, // 重试数据集嵌入迁移
  ...
  RAG_REPROCESS_FILE: "/rag/datasets/files/reprocess", // 强制重启文件预处理
  ...
  RAG_SESSION: "/rag/search/session",
  RAG_STREAM_CHAT_SESSION: (sessionId: string) => `/rag/search/session/${sessionId}`,
  RAG_USER_RAG_SESSION: (userRagId: string) => `/rag/search/user-rag/${userRagId}/session`,
  ...
  BATCH_DELETE_MEMORY: "/portal/memory/items/batch-delete", // 批量删除记忆
}
