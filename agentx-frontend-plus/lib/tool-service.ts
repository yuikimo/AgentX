...
import { Tool, ToolVersion, ApiResponse, GetMarketToolsParams, PublishToolToMarketParams, ConfigureToolParams } from "@/types/tool"
...
// 配置已安装工具的私有安装变量
export async function configureInstalledTool(toolId: string, params: ConfigureToolParams): Promise<ApiResponse<any>> {
  try {
    return await httpClient.put(API_ENDPOINTS.CONFIGURE_INSTALLED_TOOL(toolId), params)
  } catch (error) {
    return {
      code: 500,
      message: "配置工具失败",
...
export const configureInstalledToolWithToast = withToast(
  configureInstalledTool,
  {
    successTitle: "配置成功",
    errorTitle: "配置失败",
    showSuccessToast: true
