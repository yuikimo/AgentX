...
// 提前预热助理绑定的 MCP 工具。失败不影响页面交互。
export async function prewarmAgentTools(agentId: string): Promise<ApiResponse<null>> {
  try {
    return await httpClient.post<ApiResponse<null>>(
      `/agents/sessions/${agentId}/prewarm`
    )
  } catch (error) {
    return {
      code: 500,
      message: error instanceof Error ? error.message : "工具预热失败",
      data: null,
      timestamp: Date.now(),
    }
  }
...
export function prewarmAgentToolsSilently(agentId: string) {
  void prewarmAgentTools(agentId)
}
