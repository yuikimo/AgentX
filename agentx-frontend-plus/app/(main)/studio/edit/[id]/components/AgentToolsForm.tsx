          ...
          {isLoadingTools ? (
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              {installedTools.map((tool) => {
                ...
                const isUnconfigured = tool.configStatus === "UNCONFIGURED";
                
                return (
                  <div
                    key={tool.id}
                    className={`border rounded-lg p-4 transition-all ${
                      isUnconfigured ? "cursor-not-allowed opacity-60" : "cursor-pointer"
                    } ${
                      isSelected ? "border-blue-500 bg-blue-50" : "hover:border-gray-300"
                    }`}
                    onClick={() => !isUnconfigured && onToolClick(tool)}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium">{tool.name}</h3>
                        {isUnconfigured && (
                          <p className="mt-1 text-xs text-amber-600">请先在工具中心完成配置</p>
                        )}
                      </div>
                      <Switch 
                        checked={isSelected} 
                        onCheckedChange={() => !isUnconfigured && toggleTool(tool)}
