...
import { ConfigureToolDialog } from "@/components/tool/configure-tool-dialog"
...
export default function ToolsPage() {
  ...
  const [toolToConfigure, setToolToConfigure] = useState<UserTool | null>(null);
  ...
  const handleOpenConfigureDialog = (tool: UserTool, event?: React.MouseEvent) => {
    if (event) {
      event.stopPropagation();
    }
    ...
    <div className="py-6 min-h-screen bg-gray-50">
      <div className="container max-w-7xl mx-auto px-2">
        ...
        <InstalledToolsSection
          ...
          onToolClick={openUserToolDetail}
          onDeleteClick={openDeleteConfirm}
          onConfigureClick={handleOpenConfigureDialog}
        />
        ...
        <ConfigureToolDialog
          ...
          toolId={toolToConfigure?.toolId || toolToConfigure?.id}
          version={toolToConfigure?.current_version || toolToConfigure?.currentVersion || "0.0.1"}
          toolName={toolToConfigure?.name}
          installFields={toolToConfigure?.installFields}
          onSuccess={fetchUserTools}
        />
      </div>
    </div>
  )
