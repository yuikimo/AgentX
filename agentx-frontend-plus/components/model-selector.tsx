...
import { getAllModels, getUserSettings, updateUserSettings, type Model, type UserSettingsConfig } from "@/lib/user-settings-service"
...
export default function ModelSelector({
  ...
  showLabel = true,
  labelText = "预览默认模型（全局）",
  descriptionText = "修改后会影响未绑定专属模型的助理与预览",
  disabled = false,
}: ModelSelectorProps) {
  // 默认模型状态
  ...
  const handleChangeDefaultModel = async (modelId: string) => {
    ...
    try {
      ...
      const existingConfig: UserSettingsConfig =
        userSettingsResponse.code === 200 && userSettingsResponse.data?.settingConfig
          ? userSettingsResponse.data.settingConfig
          : {
              defaultModel: null,
      ...
      const response = await updateUserSettings({
        settingConfig: {
          ...
          defaultOcrModel: existingConfig.defaultOcrModel,
          defaultEmbeddingModel: existingConfig.defaultEmbeddingModel,
          fallbackConfig: existingConfig.fallbackConfig,
        }
      })
