...
export function CreatedRagsSection() {
  ...
  const hasMigratingDataset = datasets.some((dataset) => dataset.embeddingMigrationStatus === "MIGRATING")
  ...
  // 加载数据集
  const loadDatasets = async (page: number = 1, keyword?: string, silent: boolean = false) => {
    try {
      if (!silent) {
        setLoading(true)
        setError(null)
  ...
  // 当存在迁移中的数据集时自动刷新列表，及时反映状态变化
  useEffect(() => {
    if (!hasMigratingDataset) return

    const timer = setInterval(() => {
      loadDatasets(pageData.current, debouncedQuery, true)
    }, 5000)

    return () => clearInterval(timer)
  }, [hasMigratingDataset, pageData.current, debouncedQuery])
    ...
    <div className="mb-8 bg-white p-6 rounded-lg shadow-sm border border-gray-100">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {hasMigratingDataset && (
            <p className="text-xs text-muted-foreground mt-1">检测到迁移任务进行中，列表每5秒自动刷新</p>
          )}
        </div>
      </div>
