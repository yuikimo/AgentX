...
export default function MessageFileDisplay({
  ...
  const displayItems: Array<Partial<ConversationAttachment> & { url: string }> = attachments && attachments.length > 0
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {displayItems.map((item, index) => {
        const url = item.url
        const fileName = item.name || getFileNameFromUrl(url)
        const fileType = item.kind === 'IMAGE' || item.contentType?.startsWith('image/') ? 'image' : getFileType(url)
