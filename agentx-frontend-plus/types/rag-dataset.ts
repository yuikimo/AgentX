...
export interface RagDataset {
  ...
  embeddingMigrationStatus?: EmbeddingMigrationStatus;
  embeddingMigrationError?: string | null;
  fileCount: number;
  createdAt: string;
  updatedAt: string;
}

export type EmbeddingMigrationStatus = "READY" | "MIGRATING" | "FAILED" | string;
...
export interface SwitchDatasetEmbeddingModelRequest {
  embeddingModelId: string;
}
...
export interface ProcessFileRequest {
  ...
  sessionId?: string; // 会话ID（可选，用于后端优先解析会话/工作区模型）
}
...
export interface RagSessionDTO {
  sessionId: string;
}
